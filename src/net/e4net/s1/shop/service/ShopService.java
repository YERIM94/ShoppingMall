package net.e4net.s1.shop.service;
 ...
 */
@Service("ShopService")
public class ShopService extends TestService {
	 ...
	 * @throws Exception
	 */
	public int registerGoods(goodsRegisterDVO dvo) throws Exception {
		SqlSession session = null;
		int reg = -1;
		try {
			session = openSession(true);
			reg = session.insert("shop.insertGoods", dvo);
			//session.insert("shop.inserGoodsSno", dvo); //상품일련번호 생성
			} catch (Throwable t) {
			daoException(t);
		} finally {
			closeSession(session);
